package io.chefbook.libs.crypto.encryption

import io.chefbook.libs.crypto.encryption.models.AsymmetricKey
import io.chefbook.libs.crypto.encryption.models.SymmetricCipherData
import io.chefbook.libs.crypto.encryption.models.SymmetricKey
import io.chefbook.libs.crypto.encryption.models.createAsymmetricPrivateKey
import io.chefbook.libs.crypto.encryption.models.createAsymmetricPublicKey
import io.chefbook.libs.utils.uuid.generateUUID
import io.ktor.util.decodeBase64Bytes
import kotlin.test.Test
import kotlin.test.assertContentEquals
import kotlin.test.assertEquals

class HybridCryptorTest {

  @Test
  fun symmetricKeyGeneration() {
    HybridCryptor.generatePasswordSymmetricKey()
  }

  @Test
  fun passwordSymmetricKeyGeneration() {
    val key = HybridCryptor.generatePasswordSymmetricKey(
      password = testPBEPassword,
      salt = testPBESalt,
    )

    assertContentEquals(
      expected = testPBEKey,
      actual = key.raw,
    )
  }

  @Test
  fun asymmetricKeyGeneration() {
    HybridCryptor.generateAsymmetricKey()
  }

  @Test
  fun ivGeneration() {
    val generatedIV = HybridCryptor.generateIV(seed = externalIVSeed)
    assertContentEquals(externalIV, generatedIV)
  }

  @Test
  fun saltGeneration() {
    HybridCryptor.generateSalt()
  }

  @Test
  fun encryptionDecryptionByRandomSymmetricKey() {
    val originPlaintext = generateUUID()
    val key = HybridCryptor.generatePasswordSymmetricKey()

    val cipherData = HybridCryptor.encryptBySymmetricKey(
      plaintext = originPlaintext.encodeToByteArray(),
      key = key,
      iv = HybridCryptor.generateRandomIV(),
    )
    val decryptedPlaintext = HybridCryptor.decryptBySymmetricKey(
      cipherData = cipherData,
      key = key,
    )

    assertEquals(
      expected = originPlaintext,
      actual = decryptedPlaintext.decodeToString(),
    )
  }

  @Test
  fun decryptionByExternalSymmetricKey() {
    val decryptedPlaintext = HybridCryptor.decryptBySymmetricKey(
      cipherData = externalSymmetricCipherData,
      key = externalSymmetricKey,
    )

    assertEquals(
      expected = externalPlaintext,
      actual = decryptedPlaintext.decodeToString(),
    )
  }

  @Test
  fun encryptionByPasswordSymmetricKey() {
    val originPlaintext = generateUUID()
    val password = generateUUID()
    val salt = HybridCryptor.generateSalt()

    val encryptionKey = HybridCryptor.generatePasswordSymmetricKey(
      password = password,
      salt = salt,
    )
    val cipherData = HybridCryptor.encryptBySymmetricKey(
      plaintext = originPlaintext.encodeToByteArray(),
      key = encryptionKey,
      iv = HybridCryptor.generateRandomIV(),
    )

    val decryptionKey = HybridCryptor.generatePasswordSymmetricKey(
      password = password,
      salt = salt,
    )
    val decryptedPlaintext = HybridCryptor.decryptBySymmetricKey(
      cipherData = cipherData,
      key = decryptionKey,
    )

    assertEquals(
      expected = originPlaintext,
      actual = decryptedPlaintext.decodeToString(),
    )
  }

  @Test
  fun encryptionDecryptionByRandomAsymmetricKey() {
    val originPlaintext = generateUUID()
    val key = HybridCryptor.generateAsymmetricKey()

    val ciphertext = HybridCryptor.encryptByAsymmetricKey(
      plaintext = originPlaintext.encodeToByteArray(),
      key = key.public,
    )
    val decryptedPlaintext = HybridCryptor.decryptByAsymmetricKey(
      ciphertext = ciphertext,
      key = key.private,
    )

    assertEquals(
      expected = originPlaintext,
      actual = decryptedPlaintext.decodeToString(),
    )
  }

  @Test
  fun encryptionDecryptionByExternalAsymmetricKey() {
    val originPlaintext = generateUUID()
    val key = AsymmetricKey(
      public = createAsymmetricPublicKey(externalAsymmetricPublicKey),
      private = createAsymmetricPrivateKey(externalAsymmetricPrivateKey),
    )

    val ciphertext = HybridCryptor.encryptByAsymmetricKey(
      plaintext = originPlaintext.encodeToByteArray(),
      key = key.public,
    )
    val decryptedPlaintext = HybridCryptor.decryptByAsymmetricKey(
      ciphertext = ciphertext,
      key = key.private,
    )

    assertEquals(
      expected = originPlaintext,
      actual = decryptedPlaintext.decodeToString(),
    )
  }

  @Test
  fun symmetricKeyEncryptionByAsymmetricKey() {
    val originSymmetricKey = HybridCryptor.generatePasswordSymmetricKey()
    val asymmetricKey = HybridCryptor.generateAsymmetricKey()

    val ciphertext = HybridCryptor.encryptSymmetricKeyByPublicKey(
      symmetricKey = originSymmetricKey,
      publicKey = asymmetricKey.public,
    )
    val decryptedSymmetricKey = HybridCryptor.decryptSymmetricKeyByPrivateKey(
      ciphertext = ciphertext,
      key = asymmetricKey.private,
    )

    assertContentEquals(
      expected = originSymmetricKey.raw,
      actual = decryptedSymmetricKey.raw,
    )
  }

  @Test
  fun asymmetricKeyEncryptionBySymmetricKey() {
    val originAsymmetricKey = HybridCryptor.generateAsymmetricKey()
    val symmetricKey = HybridCryptor.generatePasswordSymmetricKey()

    val cipherData = HybridCryptor.encryptPrivateKeyBySymmetricKey(
      privateKey = originAsymmetricKey.private,
      symmetricKey = symmetricKey,
      iv = HybridCryptor.generateRandomIV(),
    )
    val decryptedAsymmetricKey = HybridCryptor.decryptAsymmetricKeyBySymmetricKey(
      cipherData = cipherData,
      key = symmetricKey,
    )

    assertContentEquals(
      expected = originAsymmetricKey.public.raw,
      actual = decryptedAsymmetricKey.public.raw,
    )
    assertContentEquals(
      expected = originAsymmetricKey.private.raw,
      actual = decryptedAsymmetricKey.private.raw,
    )
  }

  companion object {
    private val externalSymmetricKey = SymmetricKey(
      raw = "SVYDKXGiucwo356fJ5o2GJ/rv24hgfK6tH+nDKORz7Q=".decodeBase64Bytes(),
    )

    private val externalIVSeed = "test_iv_seed"
    private val externalIV = "/NVqFaK4i1feqbrN".decodeBase64Bytes()

    private val externalPlaintext =
      "Test Тест!#@$%^'*()ぁ \uD83E\uDD17 \uD83D\uDD25"
    private val externalSymmetricCipherData: SymmetricCipherData
      get() {
        val externalCiphertext =
          "8Seh69G0lG34H6T4QQ3rFPj18r3aYtXyROkvEfuDM7wPBhQy".decodeBase64Bytes()
        val externalTag = "34Z2ESWBFTcm8RqVfUta/w==".decodeBase64Bytes()
        return SymmetricCipherData(
          combinedData = externalCiphertext + externalTag,
          iv = externalIV,
        )
      }

    private val testPBEPassword = "test_password"
    private val testPBESalt = "test_salt".encodeToByteArray()
    private val testPBEKey = "x4UMwwIDk/uV727t/KqorGhhgW8z5+VDfjoK8kqT7cY=".decodeBase64Bytes()

    private val externalAsymmetricPublicKey =
      "".decodeBase64Bytes()
    private val externalAsymmetricPrivateKey =
      "".decodeBase64Bytes()
  }
}
