...
import com.cactusknights.chefbook.legacy.adapters.CookingEditAdapter
import com.cactusknights.chefbook.legacy.adapters.IngredientEditAdapter
import com.cactusknights.chefbook.R
import com.cactusknights.chefbook.databinding.ActivityCommitRecipeBinding
import com.cactusknights.chefbook.legacy.dialogs.CategoriesDialog
import com.cactusknights.chefbook.legacy.dialogs.ConfirmDialog
...
class RecipeCommitActivity: AppCompatActivity() {
    ...
    private fun commitRecipe() {
        ...
        when {
            ...
            confirmedRecipe.cooking.isEmpty() -> { this.showToast(resources.getString(R.string.enter_step)) }
                ...
                if (confirmedRecipe.id == 0) UuuserViewModel.addRecipe(confirmedRecipe, ::onCommitRecipeCallback)
    ...
    private fun getRecipe(): Recipe {
        ...
        return Recipe(
            id = originalRecipe?.id ?: 0, name = nameText,
            isFavourite = originalRecipe?.isFavourite ?: false,
            creationTimestamp = originalRecipe?.creationTimestamp ?: Date(),
            categories = categories,

            servings = if (servingsText.isNotEmpty()) servingsText.toInt() else 1,
            time = if (timeText.isNotEmpty()) timeText.toInt() else 15,
