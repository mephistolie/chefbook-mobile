...
import com.cactusknights.chefbook.legacy.interfaces.ContentListener
...
class FirebaseContentRepository: ContentListener {
    ...
    companion object: ContentProvider {
        ...
        override suspend fun updateRecipe(recipe: Recipe, callback: (isUpdated: Boolean) -> Unit) {
            ...
            val user = auth.currentUser
            if (user != null) {
                firestore.collection("users").document(user.uid).collection("recipes").document(recipe.id.toString()).set(recipe)
                    .addOnSuccessListener { callback(true) }
                    .addOnFailureListener { callback(false) }
            }
        }
        ...
        override suspend fun deleteRecipe(recipe: Recipe, callback: (isDeleted: Boolean) -> Unit) {
            ...
            val user = auth.currentUser
            if (user != null) {
                firestore.collection("users").document(user.uid).collection("recipes").document(recipe.id.toString()).delete()
                    .addOnSuccessListener { callback(true) }
                    .addOnFailureListener { callback(false) }
            }
        }
        ...
        override suspend fun setRecipeFavoriteStatus(recipe: Recipe) {
            ...
            if (user != null)
                firestore.collection("users")
                    .document(user.uid).collection("recipes").document(recipe.id.toString()).update(mapOf("favourite" to recipe.isFavourite))
        }
        ...
        private fun convertLegacyRecipe(legacyRecipe: LegacyRecipe?): Recipe? {
            ...
            return Recipe(
                ...
                creationTimestamp = legacyRecipe.creationDate,
                categories = legacyRecipe.categories,

                servings = legacyRecipe.servings,
                time = legacyRecipe.time.toInt(),
