package com.cactusknights.chefbook.migration
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        if (oldVersion == 1) {
            ...
            if(result!!.moveToFirst()) {
                do {
                    val category : String = result.getString(result.getColumnIndex(COL_TITLE))
                    var count = 0
        ...
        if(result.moveToFirst()) {
            do {
                ...
                val recipe = Recipe(
                    ...
                    servings = result.getString(result.getColumnIndex(COL_AMOUNT)).toInt(),
                    time = result.getString(result.getColumnIndex(COL_TIME)).toString() + " m",
                    calories = result.getString(result.getColumnIndex(COL_CALORICITY)).toInt(),

                    ingredients = getIngredientBySql(result.getString(result.getColumnIndex(COL_INGREDIENTS))),
                    cooking = cooking)
